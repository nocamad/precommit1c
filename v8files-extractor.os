...
Функция ЗапускВКоманднойСтроке()
	...
	Попытка
		...
		Если ИспользоватьКонфигуратор Тогда
			КонтекстКонфигуратора = Новый Структура("КлючСоединенияСБазой, ИмяПользователя, Пароль, КлючРазрешенияЗапуска, КодЯзыка, КодЯзыкаСеанса");
			КонтекстКонфигуратора.КлючСоединенияСБазой = Аргументы.ЗначенияПараметров["--ibconnectionstring"];
			КонтекстКонфигуратора.ИмяПользователя = Аргументы.ЗначенияПараметров["--ibuser"];
			КонтекстКонфигуратора.Пароль = Аргументы.ЗначенияПараметров["--ibpwd"];
			КонтекстКонфигуратора.КлючРазрешенияЗапуска = "";
			КонтекстКонфигуратора.КодЯзыка = "";
			КонтекстКонфигуратора.КодЯзыкаСеанса = "";
		КонецЕсли;
...
Процедура ДобавитьОписаниеКомандыДекомпилировать(Знач Парсер)
	...
	Парсер.ДобавитьИменованныйПараметрКоманды(ОписаниеКоманды, "--ibconnectionstring", "Строка соединения с ИБ");
	Парсер.ДобавитьИменованныйПараметрКоманды(ОписаниеКоманды, "--ibuser", "Пользователь БД");
	Парсер.ДобавитьИменованныйПараметрКоманды(ОписаниеКоманды, "--ibpwd", "Пароль БД");
...
Функция РазобратьФайлВнутр(Знач Файл, Знач КаталогВыгрузки, Знач КаталогКорень)
	...
	ИначеЕсли ИспользоватьКонфигуратор Тогда
		ЗапуститьРаспаковкуОбработкиШтатно(Файл, ПапкаИсходников);
	Иначе
...
Процедура ЗапуститьРаспаковкуОбработкиШтатно(Знач Файл, Знач ПапкаИсходников)

		Лог.Отладка("Запускаем распаковку файла обработки");
		...
		Лог.Отладка("Разбор обработки '%1' в исходники в каталог '%2'", ИмяОбработки, ПапкаИсходников.ПолноеИмя);

		Параметры = Конфигуратор.ПолучитьПараметрыЗапуска();
		Параметры.Добавить("/DumpExternalDataProcessorOrReportToFiles """ + ПапкаИсходников.ПолноеИмя + """ """ + Файл.ПолноеИмя + """");
		//Параметры.Добавить("-Format """ + ФорматВыгрузки + """"); // Plain – линейный формат; Hierarchical – иерархический формат (по умолчанию).
